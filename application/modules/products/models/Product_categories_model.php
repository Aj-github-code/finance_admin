    ...
    class Product_categories_model extends CI_Model
    {
        ...
        function get_data($conditions=[], $limit='', $offset=0, $allcount='')
        {
            ...
            if(isset($_GET['search']) && !empty($_GET['search']) && $_GET['search'] != 'undefined')
            {
                ...
                $like_sql   = ' AND
                                (
                                    ...
                                    OR c.order LIKE "%'.$term.'%" ESCAPE "!"
                                    OR c.description LIKE "%'.$term.'%" ESCAPE "!"
                                    OR c.created_on LIKE "%'.$term.'%" ESCAPE "!"
                                    OR pc.name LIKE "%'.$term.'%" ESCAPE "!"
                                )
                            ';
            }
            ...
            if(isset($_GET['custom_search']) && !empty($_GET['custom_search']))
            {
                ...
                if($parent_ids != '')
                {
                    $where      .= ' AND c.parent_id IN ('.implode(',', $parent_ids).')';
                }
            ...
            if(isset($_GET['order']) && !empty($_GET['order']) && isset($_GET['sort']) && !empty($_GET['sort']))
            {
                ...
                else if($_GET['sort'] == 'order')
                {
                    $sort_by = 'c.order';
                }
                else if($_GET['sort'] == 'parent_category_name')
                {
                    $sort_by = 'pc.name';
                }
            ...
            $this->db->from($this->table_product_categories.' c');
            $this->db->join($this->table_product_categories.' pc', 'pc.id=c.parent_id', 'LEFT');
            $this->db->where($where);
        ...
        function get_all_categories($post_data='')
        {
          // print_r($post_data);exit;
            $this->db->select('*');
            $this->db->from($this->table_product_categories);
            $this->db->where('status', 1);
            if(!empty($post_data['parent_id'])){
                if(is_array($post_data['parent_id'])){
                    $parent_id = implode(",",$post_data['parent_id']);
                }else{
        ...
        function save($id='', $post_data=[])
        {
            ...
            if(!empty($post_data))
            {
                ...
                if(!empty($id))
                {
                    ...
                    $this->db->where('id', $id);
                    $this->db->update($this->table_product_categories, $save);
                }
                else
                {
                    ...
                    $this->db->insert($this->table_product_categories, $save);
                }

                if($this->db->affected_rows())
                {
                    $msg = ['error' => 0, 'message' => 'Product category successfully saved'];
                }
                else
                {
                    $msg = ['error' => 1, 'message' => 'Unable to save product category'];
                }
            }
            return $msg;
        }
        ...
        function change($id='', $status)
        {
            ...
            $this->db->where('id', $id);
            $this->db->update($this->table_product_categories, $update);
