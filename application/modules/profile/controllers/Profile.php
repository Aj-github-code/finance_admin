...
class Profile extends MY_Controller {
        ...
        if($this->form_validation->run($this) === TRUE)
        {
            if($this->input->post())
            {
                ...
                if(isset($response['error']) && $response['error'] == 0)
                {
	                ...
	                {
	                	$this->session->set_userdata('full_name', $response['user']['full_name']);
	                	$this->session->set_userdata('email', $response['user']['email']);
	                	$this->session->set_userdata('mobile', $response['user']['mobile']);
	                	$this->session->set_userdata('dob', $response['user']['dob']);
	                	$this->session->set_userdata('address', $response['user']['address']);
	                }
                }
    ...
    {
        $this->form_validation->set_rules('full_name', 'Name', 'required|max_length[64]|trim|xss_clean|strip_tags');
